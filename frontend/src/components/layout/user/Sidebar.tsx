import { 
  Home, ShoppingCart, Package, FileText, ShieldCheck, MessageCircle,
  AlertTriangle, LifeBuoy, Settings, LogOut
} from 'lucide-react';
...
export default function SidebarUser({ collapsed = false }: SidebarProps) {
  const navItems = [
    { icon: <Home size={20} />, label: 'Dashboard', active: true },
    { icon: <ShoppingCart size={20} />, label: 'Browse Listings' },
    { icon: <Package size={20} />, label: 'My Trades' },
    { icon: <FileText size={20} />, label: 'My Listings' },
    { icon: <ShieldCheck size={20} />, label: 'Verification' },
  ...
  return (
    <div className={`h-screen flex flex-col transition-all duration-300 bg-gray-50 border-r border-gray-200 ${collapsed ? 'w-20' : 'w-74'}`}>
      
      {/* Logo */}
      <div className="p-6 border-b border-gray-200 flex items-center space-x-3">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-indigo-500 text-white font-bold text-sm">
          TC
        </div>
        {!collapsed && <h1 className="text-xl font-bold text-gray-800">TradeConnect</h1>}
      </div>
